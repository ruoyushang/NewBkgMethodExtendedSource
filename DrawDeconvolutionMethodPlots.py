...
for s in range(0,len(source)):
    ...
    FilePath = "output_Jul05/Deconvolution_"+target+"_Theta2%sto%s"%(int(0.*10.),int(10.*10.))+".root";
    ...
    for e in range(0,len(energy_list)-1):
        ...
        if PlotAllEnergies:
            ...
            legends = []
            colors = []
            Hists += [Hist1D_W_CR_Data_SumSRs]
            legends += ['%s'%(target)]
            colors += [1]
            Hists += [Hist1D_W_CR_Hadr_SumSRs]
            legends += ['hadron']
            colors += [4]
            Hists += [Hist1D_W_CR_Elec_SumSRs]
            legends += ['electron (MC)']
            colors += [3]
            plotname = 'Target_W_SumCRs_RDBM_E%s'%(ErecS_lower_cut)
            title = 'MSCW'
            MakeChi2Plot(Hists,legends,colors,title,plotname,True,False,MSCW_lower_cut,MSCW_blind_cut,-1)
            Hists = []
            legends = []
            colors = []
            Hists += [Hist1D_L_CR_Data_SumSRs]
            legends += ['%s'%(target)]
            colors += [1]
            Hists += [Hist1D_L_CR_Hadr_SumSRs]
            legends += ['hadron']
            colors += [4]
            Hists += [Hist1D_L_CR_Elec_SumSRs]
            legends += ['electron (MC)']
            colors += [3]
            plotname = 'Target_L_SumCRs_RDBM_E%s'%(ErecS_lower_cut)
            title = 'MSCL'
            MakeChi2Plot(Hists,legends,colors,title,plotname,True,False,MSCW_lower_cut,MSCW_blind_cut,-1)
            Hists = []
            legends = []
            colors = []
            Hists += [Hist1D_W_Data_SumSRs]
            legends += ['%s'%(target)]
            colors += [1]
            Hists += [Hist1D_W_Hadr_SumSRs]
            legends += ['hadron']
            colors += [4]
            Hists += [Hist1D_W_Elec_SumSRs]
            legends += ['electron (MC)']
            colors += [3]
            ...
            legends = []
            colors = []
            Hists += [Hist1D_L_Data_SumSRs]
            legends += ['%s'%(target)]
            colors += [1]
            Hists += [Hist1D_L_Hadr_SumSRs]
            legends += ['hadron']
            colors += [4]
            Hists += [Hist1D_L_Elec_SumSRs]
            legends += ['electron (MC)']
            colors += [3]
            ...
            legends += ['Ring']
            colors += [4]
            plotname = 'Target_W_SumSRs_ZoomInRing_E%s'%(ErecS_lower_cut)
            title = 'MSCW'
            MakeChi2Plot(Hists,legends,colors,title,plotname,True,False,MSCW_lower_cut,MSCW_blind_cut,-1)
            Hists = []
            legends = []
            ...
            legends += ['Segue1']
            colors += [4]
            plotname = 'Target_W_SumSRs_ZoomInDark_E%s'%(ErecS_lower_cut)
            title = 'MSCW'
            MakeChi2Plot(Hists,legends,colors,title,plotname,True,False,MSCW_lower_cut,MSCW_blind_cut,-1)
        ...
        cr_syst = 0
        cr_syst = abs(w_cr_data_total-w_cr_hadr_total)/w_cr_data_total
        print 'w_cr_syst = %s'%(cr_syst)
        Hist1D_W_Hadr_SumSRs = AddSystCR(Hist1D_W_Hadr_SumSRs,bin_lower,bin_upper,cr_syst)
        cr_syst = 0
        cr_syst = abs(l_cr_data_total-l_cr_hadr_total)/l_cr_data_total
        print 'l_cr_syst = %s'%(cr_syst)
        Hist1D_L_Hadr_SumSRs = AddSystCR(Hist1D_L_Hadr_SumSRs,bin_lower,bin_upper,cr_syst)
        ...
        w_weight = 1
        #if (w_bkg_err>0): w_weight = pow(1./w_bkg_err,2)
        l_weight = 1
        #if (l_bkg_err>0): l_weight = pow(1./l_bkg_err,2)
        ...
        if not bkg_total==0:
            Theta2HistScale(Hist_Hadr_Theta2_Tmp,hadr_scale,hadr_scale_err)
        else:
        ...
        if not bkg_total==0 and not old_integral==0:
            ...
            scale_err = 0
        if not bkg_total==0:
            RaDecHistScale(Hist_Hadr_Skymap_Tmp,scale,scale_err)
        else:
        ...
        if not bkg_total==0 and not old_integral==0:
            ...
            elec_scale_err = 0
        if not bkg_total==0:
            Theta2HistScale(Hist_Elec_Theta2_Tmp,elec_scale,elec_scale_err)
        else:
        ...
        if not bkg_total==0 and not old_integral==0:
            ...
            scale_err = 0
        if not bkg_total==0:
            RaDecHistScale(Hist_Elec_Skymap_Tmp,scale,scale_err)
        else:
        ...
        Theta2HistScale(Hist_Ring_Theta2_Tmp,ring_scale,ring_scale_err)
        ...
        Theta2HistScale(Hist_Dark_Theta2_Tmp,dark_scale,dark_scale_err)
    ...
    scale = 0
    scale_err = 0
    if old_integral>0 and ring_integral>0:
    ...
    legends += ['electron (MC)']
    colors += [3]
    plotname = 'Target_CRall_RDBM_MSCL'
    title = 'MSCL'
    MakeChi2Plot(Hists,legends,colors,title,plotname,True,False,MSCW_lower_cut,MSCW_blind_cut,0)
    Hists = []
    legends = []
    ...
    legends += ['electron (MC)']
    colors += [3]
    plotname = 'Target_SRall_RDBM_MSCL'
    title = 'MSCL'
    MakeChi2Plot(Hists,legends,colors,title,plotname,True,False,MSCW_lower_cut,MSCW_blind_cut,0)
    Hists = []
    legends = []
    ...
    legends += ['electron (MC)']
    colors += [3]
    plotname = 'Target_CRall_ZoomInRDBM_MSCL'
    title = 'MSCL'
    MakeChi2Plot(Hists,legends,colors,title,plotname,True,False,MSCW_lower_cut,MSCW_blind_cut,0)
    Hists = []
    legends = []
    ...
    legends += ['electron (MC)']
    colors += [3]
    plotname = 'Target_SRall_ZoomInRDBM_MSCL'
    title = 'MSCL'
    MakeChi2Plot(Hists,legends,colors,title,plotname,True,False,MSCW_lower_cut,MSCW_blind_cut,0)
    Hists = []
    legends = []
