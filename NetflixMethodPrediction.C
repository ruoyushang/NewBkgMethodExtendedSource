...
MatrixXcd GetEigenvalueMatrix(MatrixXcd mtx_input)
{
    ...
    for (int row=0;row<mtx_input.rows();row++)
    {
        for (int col=0;col<mtx_input.cols();col++)
        {
            ...
            if (row==col)
            {
                if (col>=mtx_input.cols()-NumberOfEigenvectors)
                {
                    mtx_lambda(row,col) = eigensolver_input.eigenvalues()(col).real();
                }
                else
                {
...
MatrixXcd MakeRealEigenvectors(MatrixXcd mtx_input)
{
    ...
    for (int col=0;col<mtx_input.cols();col++)
    {
        ...
        for (int row=0;row<mtx_input.rows();row++)
        {
            //mtx_output(row,col) = mtx_input(row,col)*std::polar(1.,-1.*std::arg(mtx_input(row,col)));
    ...
    for (int row=0;row<mtx_input.rows();row++)
    {
        for (int col=0;col<mtx_input.cols();col++)
        {
            ...
            if (row==col)
            {
                if (col>=mtx_input.cols()-NumberOfEigenvectors)
                {
                    mtx_lambdanu(row,col) = eigensolver_input.eigenvalues()(col).real()/mtx_H(row,col).real();
                }
                else
                {
...
void VerifyLeftRightVectorCoupling(MatrixXcd mtx_input, MatrixXcd mtx_init, int entry_cutoff)
{
    std::cout << "++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++" << std::endl;
    std::cout << "entry_cutoff = " << entry_cutoff << std::endl;
    ComplexEigenSolver<MatrixXcd> eigensolver_input;
    eigensolver_input = ComplexEigenSolver<MatrixXcd>(mtx_init);
    ComplexEigenSolver<MatrixXcd> eigensolver_input_transpose;
    eigensolver_input_transpose = ComplexEigenSolver<MatrixXcd>(mtx_init.transpose());
    MatrixXcd mtx_U_r = eigensolver_input.eigenvectors();
    MatrixXcd mtx_U_l = eigensolver_input_transpose.eigenvectors();
    mtx_U_r = MakeRealEigenvectors(mtx_U_r);
    mtx_U_l = MakeRealEigenvectors(mtx_U_l);
    ...
    std::cout << "mtx_U_rc.col(mtx_input.cols()-1) = " << std::endl;
    std::cout << mtx_U_rc.col(mtx_input.cols()-1) << std::endl;
    MatrixXcd mtx_U_rc_new = (mtx_BigG1-mtx_lambda(mtx_input.cols()-1,mtx_input.cols()-1)*mtx_unit).inverse()*(-mtx_Smallg1*mtx_lambdanu_R_lc).col(mtx_input.cols()-1);
    ...
    std::cout << "mtx_U_lc.col(mtx_input.cols()-1) = " << std::endl;
    std::cout << mtx_U_lc.col(mtx_input.cols()-1) << std::endl;
    MatrixXcd mtx_U_lc_new = (mtx_BigG2-mtx_lambda(mtx_input.cols()-1,mtx_input.cols()-1)*mtx_unit).inverse()*(-mtx_Smallg2*mtx_lambdanu_L_rc).col(mtx_input.cols()-1);
...
void AnalyzeBigGMatrix(MatrixXcd mtx_input)
{
    std::cout << "++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++" << std::endl;

    ComplexEigenSolver<MatrixXcd> eigensolver_input;
    eigensolver_input = ComplexEigenSolver<MatrixXcd>(mtx_input);
    std::cout << "eigenvalues of original matrix:" << std::endl;
    std::cout << eigensolver_input.eigenvalues() << std::endl;
    ...
    ComplexEigenSolver<MatrixXcd> eigensolver_BigG1 = ComplexEigenSolver<MatrixXcd>(mtx_BigG1);
    std::cout << "eigenvalues of G1:" << std::endl;
    std::cout << eigensolver_BigG1.eigenvalues() << std::endl;
    VerifyHMatrix(mtx_BigG1);

    ComplexEigenSolver<MatrixXcd> eigensolver_BigG2 = ComplexEigenSolver<MatrixXcd>(mtx_BigG2);
    std::cout << "eigenvalues of G2:" << std::endl;
    std::cout << eigensolver_BigG2.eigenvalues() << std::endl;
    VerifyHMatrix(mtx_BigG2);

    ComplexEigenSolver<MatrixXcd> eigensolver_Smallg1 = ComplexEigenSolver<MatrixXcd>(mtx_Smallg1);
    std::cout << "eigenvalues of g1:" << std::endl;
    std::cout << eigensolver_Smallg1.eigenvalues() << std::endl;
    VerifyHMatrix(mtx_Smallg1);

    ComplexEigenSolver<MatrixXcd> eigensolver_Smallg2 = ComplexEigenSolver<MatrixXcd>(mtx_Smallg2);
    std::cout << "eigenvalues of g2:" << std::endl;
    std::cout << eigensolver_Smallg2.eigenvalues() << std::endl;
    VerifyHMatrix(mtx_Smallg2);
...
pair<std::complex<double>,std::complex<double>> GetIterativeMatrix(MatrixXcd mtx_input, int NthEigenvector, std::complex<double> par_R, std::complex<double> par_L)
{
    ...
    std::cout << eigensolver_iterative.eigenvalues() << std::endl;
...
MatrixXcd BuildModelMatrix()
{
    ...
    //std::cout << "mtx_eigenvector_vari = " << std::endl;
    //std::cout << mtx_eigenvector_vari.block(0,mtx_data_bkgd.cols()-3,mtx_data_bkgd.rows(),3) << std::endl;
    //std::cout << "mtx_eigenvector_inv_vari = " << std::endl;
    //std::cout << mtx_eigenvector_inv_vari.block(mtx_data_bkgd.rows()-3,0,3,mtx_data_bkgd.cols()) << std::endl;

    //MatrixXcd mtx_M1 = GetSubmatrix(mtx_data, 1);
    //MatrixXcd mtx_M2 = GetSubmatrix(mtx_data, 2);
    //MatrixXcd mtx_M3 = GetSubmatrix(mtx_data, 3);
    //MatrixXcd mtx_alpha1 = GetAlphaMatrix(mtx_data, 1);
    //MatrixXcd mtx_alpha2 = GetAlphaMatrix(mtx_data, 2);
    //MatrixXcd mtx_unit = GetUnitMatrix(mtx_M3.rows(),mtx_M3.cols());
    //MatrixXcd mtx_Smallg1 = mtx_M2*mtx_alpha1.inverse()*mtx_M1;
    //MatrixXcd mtx_BigG1 = mtx_M3-mtx_Smallg1*mtx_M3.transpose();
    //MatrixXcd mtx_Smallg2 = mtx_M1.transpose()*mtx_alpha2.inverse()*mtx_M2.transpose();
    ...
    if (correlate_left_right)
    {
        for (int nth_entry=2;nth_entry<=NumberOfEigenvectors;nth_entry++)
        {
            ...
            double lambda_largest_init = eigensolver_init.eigenvalues()(mtx_dark.cols()-1).real();
            double lambda_init = eigensolver_init.eigenvalues()(mtx_dark.cols()-nth_entry).real();
            MatrixXcd mtx_U_r_init = mtx_eigenvector_init;
            MatrixXcd mtx_U_l_init = mtx_eigenvector_inv_init.transpose();
            MatrixXcd mtx_H_init = mtx_U_l_init.transpose()*mtx_U_r_init;
            double eta_init = mtx_H_init(mtx_dark.cols()-nth_entry,mtx_dark.cols()-nth_entry).real();
            MatrixXcd mtx_U_r = mtx_eigenvector;
...
void PrintRLHMatrices(MatrixXcd mtx_input, MatrixXcd mtx_input_inv)
{
    MatrixXcd mtx_U_r = mtx_input;
    MatrixXcd mtx_U_l = mtx_input_inv.transpose();
    mtx_U_r = MakeRealEigenvectors(mtx_U_r);
    mtx_U_l = MakeRealEigenvectors(mtx_U_l);
    MatrixXcd mtx_R = mtx_U_r.transpose().conjugate()*mtx_U_r;
    MatrixXcd mtx_L = mtx_U_l.transpose().conjugate()*mtx_U_l;
    MatrixXcd mtx_H = mtx_U_l.transpose()*mtx_U_r;
    std::cout << "mtx_R = " << std::endl;
    PrintItReversely(mtx_R.block(mtx_R.rows()-3,mtx_R.cols()-3,3,3));
    std::cout << "mtx_L = " << std::endl;
    PrintItReversely(mtx_L.block(mtx_L.rows()-3,mtx_L.cols()-3,3,3));
    std::cout << "mtx_H = " << std::endl;
    PrintItReversely(mtx_H.block(mtx_H.rows()-3,mtx_H.cols()-3,3,3));
}
...
void NetflixSetInitialVariablesEigenBasis(ROOT::Math::GSLMinimizer* Chi2Minimizer, int binx_blind, int biny_blind, int fix_which, int which_to_fit)
{
    ...
    for (int NthEigenvector=1;NthEigenvector<=NumberOfEigenvectors;NthEigenvector++)
    {
        ...
        for (int entry=0;entry<NumberOfEigenvectors;entry++)
        {
            ...
            //if (NthEigenvector==entry+1) Chi2Minimizer->SetVariableLimits(first_index+entry,0.,0.);
        }
...
MatrixXcd EigenDecompositionMethod(MatrixXcd mtx_input, int LeftOrRight, int nth_entry)
{
    // LeftOrRight = 1, eigenvector inverse
    // LeftOrRight = 2, eigenvector
    ...
    MatrixXcd mtx_l_init = eigensolver_input.eigenvectors().inverse().transpose();
    ...
    if (LeftOrRight==1)
    {
        ...
        for (int row=0;row<mtx_M3.rows();row++)
        {
            ...
            mtx_Big.block(first_idx,0,mtx_input.rows(),mtx_input.cols()) = mtx_unit*mtx_lambda(mtx_input.rows()-nth_entry,mtx_input.cols()-nth_entry)*mtx_rc_init(mtx_M3.rows()-row-1,mtx_input.cols()-nth_entry);
        }
    ...
    else
    {
        ...
        for (int col=0;col<mtx_M3.cols();col++)
        {
            ...
            mtx_Big.block(first_idx,0,mtx_input.cols(),mtx_input.rows()) = mtx_unit*mtx_lambda(mtx_input.rows()-nth_entry,mtx_input.cols()-nth_entry)*mtx_lc_init(mtx_M3.cols()-col-1,mtx_input.rows()-nth_entry);
        }
        mtx_r_vari.col(mtx_r_vari.cols()-nth_entry) = mtx_Big.bdcSvd(ComputeThinU | ComputeThinV).solve(vtr_Delta);
        mtx_r_final += mtx_r_vari;
    }

    mtx_output = mtx_r_final*mtx_lambda*mtx_l_final.transpose();

    return mtx_output;
}
MatrixXcd SpectralDecompositionMethod(MatrixXcd mtx_input, int LeftOrRight, int entry_cutoff)
{
    // LeftOrRight = 1, left
    // LeftOrRight = 2, right

    MatrixXcd mtx_r_init = mtx_eigenvector;
    MatrixXcd mtx_S = mtx_eigenvalue;
    MatrixXcd mtx_l_init = mtx_eigenvector_inv.transpose();
    ...
    MatrixXcd mtx_remain = MatrixXcd::Zero(mtx_input.rows(),mtx_input.cols());
    ...
    //int row_size_big = mtx_input.rows()*mtx_input.cols()-mtx_M0.rows()*mtx_M0.cols();
    ...
    //MatrixXd mtx_Big = MatrixXd::Zero(row_size_big,1*mtx_input.cols());
    //VectorXd vtr_l_vari_big = VectorXd::Zero(1*mtx_input.cols());
    //VectorXd vtr_r_vari_big = VectorXd::Zero(1*mtx_input.cols());
    MatrixXd mtx_unit = GetUnitMatrix(mtx_input.rows(),mtx_input.cols()).real();
    ...
    if (LeftOrRight==1)
    {
        ...
        //for (int row=0;row<mtx_M1.rows();row++)
        //{
        //    int first_idx = row*(mtx_M1.cols())+mtx_M3.rows()*(mtx_M2.cols()+mtx_M3.cols());
        //    vtr_Delta.segment(first_idx,mtx_M1.cols()) = (mtx_M1-mtx_M1_init).transpose().col(mtx_M1.rows()-row-1).real();
        //}
        for (int nth_entry=1; nth_entry<=entry_cutoff; nth_entry++)
        //for (int nth_entry=entry_cutoff; nth_entry<=entry_cutoff; nth_entry++)
        {
            for (int row=0;row<mtx_M3.rows();row++)
            {
            ...
            //for (int row=0;row<mtx_M1.rows();row++)
            //{
            //    int first_idx_row = row*(mtx_input.cols())+mtx_M3.rows()*(mtx_input.cols());
            //    int first_idx_col = (nth_entry-1)*(mtx_input.rows());
            //    //int first_idx_col = 0;
            //    mtx_Big.block(first_idx_row,first_idx_col,mtx_input.rows(),mtx_M1.rows()) = mtx_unit*mtx_S(mtx_input.rows()-nth_entry,mtx_input.cols()-nth_entry).real()*mtx_rg_init(mtx_M1.rows()-row-1,mtx_input.cols()-nth_entry).real();
            //}
        }
        ...
        mtx_l_final += mtx_l_vari;
        //MatrixXcd mtx_R_S_l_transpose = mtx_R*GetEigenvalueWeightedVectors(mtx_S,mtx_l_vari).transpose();
        //VectorXcd vtr_R_S_l = mtx_R_S_l_transpose.transpose().col(mtx_R_S_l_transpose.cols()-nth_entry);
        //VectorXcd vtr_r_vari = (mtx_input.transpose().bdcSvd(ComputeThinU | ComputeThinV).solve(vtr_R_S_l)).conjugate();
        //std::cout << "mtx_l_vari.col(mtx_l_vari.cols()-nth_entry) = " << std::endl;
        //std::cout << mtx_l_vari.col(mtx_l_vari.cols()-nth_entry) << std::endl;
        //std::cout << "vtr_r_vari = " << std::endl;
        //std::cout << vtr_r_vari << std::endl;
        //std::cout << "mtx_r_init.col(mtx_r_init.cols()-nth_entry) = " << std::endl;
        //PrintItReversely(mtx_r_init.col(mtx_r_init.cols()-nth_entry));
        //mtx_r_final.col(mtx_r_final.cols()-nth_entry) += vtr_r_vari;
    }
    else
    {
        for (int col=0;col<mtx_M3.cols();col++)
        ...
        //for (int col=0;col<mtx_M2.cols();col++)
        //{
        //    int first_idx = col*(mtx_M2.rows())+mtx_M3.cols()*(mtx_M1.rows()+mtx_M3.rows());
        //    vtr_Delta.segment(first_idx,mtx_M2.rows()) = (mtx_M2-mtx_M2_init).col(mtx_M2.cols()-col-1).real();
        //}
        for (int nth_entry=1; nth_entry<=entry_cutoff; nth_entry++)
        //for (int nth_entry=entry_cutoff; nth_entry<=entry_cutoff; nth_entry++)
        {
            ...
            //for (int col=0;col<mtx_M2.cols();col++)
            //{
            //    int first_idx_row = col*(mtx_input.rows())+mtx_M3.cols()*(mtx_input.rows());
            //    int first_idx_col = (nth_entry-1)*(mtx_input.cols());
            //    //int first_idx_col = 0;
            //    mtx_Big.block(first_idx_row,first_idx_col,mtx_input.cols(),mtx_M2.cols()) = mtx_unit*mtx_S(mtx_input.rows()-nth_entry,mtx_input.cols()-nth_entry).real()*mtx_lg_init(mtx_M2.cols()-col-1,mtx_input.rows()-nth_entry).real();
            //}
        }
        ...
        mtx_r_final += mtx_r_vari;
        //MatrixXcd mtx_L_S_r_transpose = mtx_L*GetEigenvalueWeightedVectors(mtx_S,mtx_r_init).transpose();
        //VectorXcd vtr_L_S_r = mtx_L_S_r_transpose.transpose().col(mtx_L_S_r_transpose.cols()-nth_entry);
        //VectorXcd vtr_l_new = (mtx_input.bdcSvd(ComputeThinU | ComputeThinV).solve(vtr_L_S_r)).conjugate();
        //std::cout << "vtr_l_new = " << std::endl;
        //std::cout << vtr_l_new << std::endl;
        //std::cout << "mtx_l_init.col(mtx_l_init.cols()-nth_entry) = " << std::endl;
        //PrintItReversely(mtx_l_init.col(mtx_l_init.cols()-nth_entry));
        //mtx_l_final.col(mtx_l_final.cols()-nth_entry) = vtr_l_new;
    }
    ...
    if (correlate_left_right)
    {
        ...
        {
            double eta_init = mtx_H_init(mtx_dark.cols()-nth_entry,mtx_dark.cols()-nth_entry).real();
            double eta_final = mtx_H_final(mtx_dark.cols()-nth_entry,mtx_dark.cols()-nth_entry).real();
            double old_S = mtx_S(mtx_dark.cols()-nth_entry,mtx_dark.cols()-nth_entry).real();
            double new_S = old_S*eta_init/eta_final;
            mtx_S_final(mtx_dark.cols()-nth_entry,mtx_dark.cols()-nth_entry) = new_S;
        }
    }

    //mtx_output = mtx_r_final*mtx_S_final*mtx_l_final.transpose() + mtx_remain;
...
void NetflixMethodPrediction(string target_data, double PercentCrab, double tel_elev_lower_input, double tel_elev_upper_input, bool isON, double MSCW_cut_input, double MSCL_cut_input, int rank)
{
    ...
    for (int e=0;e<N_energy_bins;e++) 
    {
        ...
        //VerifyLeftRightVectorCoupling(mtx_data,mtx_data,3);
        //VerifyLeftRightVectorCoupling(mtx_data,mtx_dark,3);
        //SolveEigenvectorsIteratively(mtx_data, mtx_dark, 3);
        //AnalyzeBigGMatrix(mtx_data);
        ...
        //mtx_data_bkgd = SpectralDecompositionMethod(mtx_dark, 1, 2);
        //std::cout << "chi2 = " << GetChi2Function(mtx_data_bkgd) << std::endl;
        //mtx_data_bkgd = SpectralDecompositionMethod(mtx_data_bkgd, 2, 2);
        //std::cout << "chi2 = " << GetChi2Function(mtx_data_bkgd) << std::endl;
        //for (int iteration=0;iteration<20;iteration++)
        //{
        //    mtx_data_bkgd = SpectralDecompositionMethod(mtx_data_bkgd, 1, 2);
        //    std::cout << "chi2 = " << GetChi2Function(mtx_data_bkgd) << std::endl;
        //    mtx_data_bkgd = SpectralDecompositionMethod(mtx_data_bkgd, 2, 2);
        //    std::cout << "chi2 = " << GetChi2Function(mtx_data_bkgd) << std::endl;
        //}
        ...
        //mtx_data_bkgd = EigenDecompositionMethod(mtx_dark, 1, 1);
        //mtx_data_bkgd = EigenDecompositionMethod(mtx_data_bkgd, 2, 1);
        ...
        std::cout << "R, L, H of mtx_dark:" << std::endl;
        PrintRLHMatrices(mtx_eigenvector_dark,mtx_eigenvector_inv_dark);
        std::cout << "R, L, H of mtx_data_bkgd:" << std::endl;
        PrintRLHMatrices(mtx_eigenvector,mtx_eigenvector_inv);
