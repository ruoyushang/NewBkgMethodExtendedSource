...
double FitFunc_Amp[Number_of_SR] = {0.,0.,0.,0.,0.,0.};
double FitFunc_AmpErr[Number_of_SR] = {0.,0.,0.,0.,0.,0.};
double FitFunc_RMS[Number_of_SR] = {0.,0.,0.,0.,0.,0.};
double FitFunc_RMSErr[Number_of_SR] = {0.,0.,0.,0.,0.,0.};
const int Number_of_CR = 2;
double MSCL_control_cut_lower[Number_of_CR] = {1.25,1.00};
double MSCL_control_cut_upper[Number_of_CR] = {1.50,1.25};
double FitFunc_Ctr[Number_of_CR] = {0.,0.};
double FitFunc_CtrErr[Number_of_CR] = {0.,0.};
#else
// VEGAS
...
double FitFunction(Double_t *x, Double_t *par) {
    ...
    return par[0]*pow(xx-par[1],2)*exp(-0.5*pow((xx-par[1])/(par[2]),2));
}
void FindFitFunction(TH1* Hist_SR, TH1* Hist_Bkg, int whichSR, bool isBlinded)
{
        TF1 *func = new TF1("func",FitFunction,-5.,50.,3);
        func->SetParameter(0,100.);
        func->SetParameter(1,0);
        func->SetParameter(2,Hist_SR->GetRMS());
        func->SetParName(0,"Amp");
        func->SetParName(1,"Ctr");
        func->SetParName(2,"RMS");
        Hist_SR->Fit("func","","",-2.,20.0);
        if (isBlinded) 
        {
            double avg_ctr_cr = 0.;
            double avg_ctr_err = 0.;
            for (int s=0;s<Number_of_CR;s++)
            {
                avg_ctr_cr += (FitFunc_Ctr[s])/((double)Number_of_CR);
            }
            func->SetParameter(1,avg_ctr_cr);
            func->SetParLimits(1,avg_ctr_cr-0.5,avg_ctr_cr);
            func->SetParError(1,avg_ctr_err);
            Hist_SR->Fit("func","","",MSCW_cut_blind,20.0);
        }
        double amp = func->GetParameter(0);
        double amp_err = func->GetParError(0);
        double ctr = func->GetParameter(1);
        double ctr_err = func->GetParError(1);
        double rms = func->GetParameter(2);
        double rms_err = func->GetParError(2);
        if (isBlinded) 
        {
            FitFunc_Amp[whichSR] = amp;
            FitFunc_AmpErr[whichSR] = amp_err;
            FitFunc_RMS[whichSR] = rms;
            FitFunc_RMSErr[whichSR] = rms_err;
        }
        else
        {
            FitFunc_Ctr[whichSR] = ctr;
            FitFunc_CtrErr[whichSR] = ctr_err;
        }
        for (int i=0;i<Hist_Bkg->GetNbinsX();i++) {
            func->SetParameter(0,amp);
            func->SetParameter(1,ctr);
            func->SetParameter(2,rms);
            Hist_Bkg->SetBinContent(i+1,func->Eval(Hist_Bkg->GetBinCenter(i+1)));
        }
}
...
double ShiftAndNormalize(TH1* Hist_SR, TH1* Hist_BkgTemp, TH1* Hist_Bkg, double shift_begin, bool doShift, bool includeSR) {
    ...
    if (doShift) {
        for (int fit=0;fit<200;fit++) {
                double shift = shift_begin+double(fit)*0.01;  // Do not shift to the other direction!
                for (int i=0;i<Hist_SR->GetNbinsX();i++) {
...
void FitMethodForExtendedSources(string target_data, int NTelMin, int NTelMax, double elev_lower, double elev_upper, double azim_lower, double azim_upper, double theta2_cut_lower_input, double theta2_cut_upper_input, double MSCW_cut_blind_input, double MSCW_cut_upper_input, double MSCW_cut_lower_input,bool DoConverge) 
{
        ...
        for (int e=0;e<N_energy_bins;e++)
        {
            ...
            while (DoNextSublist) 
            {
                ...
                {
                    FindFitFunction(&Hist_Target_CR_MSCW.at(e).at(c),&Hist_Target_BkgCR_MSCW.at(e).at(c),c,false);
                }
                for (int c=0;c<Number_of_CR;c++)
                {
                ...
                for (int s=0;s<Number_of_SR;s++)
                {
                    FindFitFunction(&Hist_Target_SR_MSCW.at(e).at(s),&Hist_Target_BkgSR_MSCW.at(e).at(s),s,true);
                }
                for (int s=0;s<Number_of_SR;s++)
                {
                    ...
                    Hist_Target_BkgSR_MSCW_SumRuns.at(e).at(s).Add(&Hist_Target_BkgSR_MSCW.at(e).at(s));
                }
                for (int run=0;run<Sublist.size();run++)
                {
                ...
                for (int s=0;s<Number_of_SR;s++)
                {
                    std::cout << "Hist_Target_BkgSR_MSCW_SumRuns.at(e).at("<< s<< ").Integral() = " << Hist_Target_BkgSR_MSCW_SumRuns.at(e).at(s).Integral() << std::endl;
                }
            }
        }
        for (int e=0;e<N_energy_bins;e++)
        ...
        TFile OutputFile("output_Apr20/FitMethod_"+TString(target)+"_Ntel"+std::to_string(NTelMin)+"to"+std::to_string(NTelMax)+"_Elev"+std::to_string(int(Target_Elev_cut_lower))+"to"+std::to_string(int(Target_Elev_cut_upper))+"_Azim"+std::to_string(int(Target_Azim_cut_lower))+"to"+std::to_string(int(Target_Azim_cut_upper))+"_Theta2"+std::to_string(int(10.*Theta2_cut_lower))+"to"+std::to_string(int(10.*Theta2_cut_upper))+"_MSCWCut"+std::to_string(int(10.*MSCW_cut_upper))+"_MSCWBlind"+std::to_string(int(10.*MSCW_cut_blind))+ConvergeOrNot+".root","recreate");
