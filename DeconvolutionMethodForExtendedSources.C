...
double ShiftAndNormalize(TH1* Hist_SR, TH1* Hist_BkgTemp, TH1* Hist_Bkg, double shift_begin, double norm_low, double norm_up, bool doShift) {
    ...
    if (doShift) {
        for (int fit=0;fit<30;fit++) {
                ...
                double chi2_temp = GetChi2(Hist_SR, Hist_Bkg, norm_low, norm_up,true);
                if (chi2<chi2_temp) {
                    chi2 = chi2_temp;
                    shift_fit = shift;
                    scale_fit = scale;
...
void BuildSRB(TH1D* Hist_SR, TH1D* Hist_ASR1, TH1D* Hist_ASR2, TH1D* Hist_SRB) {
    ...
    for (int i=0;i<20;i++) {
        ...
        Hist_SRB->Scale(norm);
        double chi2 = GetChi2(Hist_SR, Hist_SRB, Norm_Lower, Norm_Upper, true);
        if (chi2_best<chi2) {
            chi2_best = chi2;
            scale_best = scale;
        }
...
void DeconvolutionMethodForExtendedSources(string target_data, double elev_lower, double elev_upper) {
        ...
        for (int e=0;e<N_energy_bins;e++) {
            ...
            offset_begin = Hist_Target_SRB_MSCW.at(e).GetMean()-Hist_Target_BkgTemp_MSCW.at(e).GetMean();
            offset_begin = ShiftAndNormalize(&Hist_Target_SR_MSCW.at(e),&Hist_Target_BkgTemp_MSCW.at(e),&Hist_Target_Bkg_MSCW.at(e),offset_begin,-1.,Norm_Upper,true);
        }
