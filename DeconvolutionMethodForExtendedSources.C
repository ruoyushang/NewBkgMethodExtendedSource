...
std::pair <double,double> FindRMS(TH1* Hist_Dark, TH1* Hist_SR, TH1* Hist_CR, TH1* Hist_Bkg, TH1* Hist_BkgTemp, TH1* Hist_Deconv, double scaled_shift, double rms_begin, double mean, double n_iter, bool includeSR, int chi2_type) {
    ...
    for (int n_rms = 0; n_rms<=100;n_rms++) {
        ...
        double inflation = 1.;
        //inflation = exp(-0.5*pow((rms-rms_begin)/(rms_begin),2));
        if (includeSR) inflation = 1.;
        func->SetParameter(0,rms);
        Hist_Deconv->Reset();
        //Hist_Deconv->FillRandom("func",1000000);
...
void DeconvolutionMethodForExtendedSources(string target_data, int NTelMin, int NTelMax, double elev_lower, double elev_upper, double azim_lower, double azim_upper, double theta2_cut_lower_input, double theta2_cut_upper_input, double MSCW_cut_blind_input, double MSCW_cut_upper_input, double MSCW_cut_lower_input,bool DoConverge) 
{
        ...
        MSCW_cut_blind = MSCW_cut_lower_input;
        for (int e=0;e<N_energy_bins;e++)
        {
            std::cout << "=================================================================" << std::endl;
            ...
            for (int c1=0;c1<Number_of_CR-1;c1++)
            {
                ...
                Hist_Dark_CR_MSCW_SumRuns.at(e).at(Number_of_CR-1).Add(&Hist_Dark_CR_MSCW.at(e).at(Number_of_CR-1));
                Hist_Dark_BkgCR_MSCW_SumRuns.at(e).at(Number_of_CR-1).Add(&Hist_Dark_BkgCR_MSCW.at(e).at(Number_of_CR-1));
        ...
        for (int e=0;e<N_energy_bins;e++)
        {
            ...
            for (int subrun=0;subrun<Sublist.size();subrun++)
            {
                ...
                for (int c1=0;c1<Number_of_CR-1;c1++)
                {
                    ...
                    //estimated_mean_err = target_initial_rms*(1.+1./pow(target_initial_stat,0.5));
                    //estimated_mean_err = estimated_mean_err*10./(current_sr+1.);
                    estimated_mean_err = target_initial_rms/2.;
