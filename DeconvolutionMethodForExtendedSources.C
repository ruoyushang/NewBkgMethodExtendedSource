...
double GetChi2(TH1* Hist_SR, TH1* Hist_Bkg, double norm_low, double norm_up) {
    ...
    for (int i=0;i<Hist_SR->GetNbinsX();i++) {
        ...
        if (Hist_SR->GetBinContent(i+1)==0) continue;
        chi2_temp += pow(Hist_Bkg->GetBinContent(i+1)-Hist_SR->GetBinContent(i+1),2);
        //chi2_temp += pow(Hist_Bkg->GetBinContent(i+1)-Hist_SR->GetBinContent(i+1),2)/Hist_SR->GetBinContent(i+1);
    }
...
double ShiftAndNormalize(TH1* Hist_SR, TH1* Hist_BkgTemp, TH1* Hist_Bkg, double shift_begin, double norm_low, double norm_up, bool doShift) {
    ...
    if (doShift) {
        for (int fit=0;fit<20;fit++) {
                double shift = shift_begin-1.+fit*0.1;
                for (int i=0;i<Hist_SR->GetNbinsX();i++) {
...
void DeconvolutionMethodForExtendedSources(string target_data, double elev_lower, double elev_upper) {
        ...
        for (int e=0;e<N_energy_bins;e++) {
            ...
            for (int n_rebins=0;n_rebins<5;n_rebins++) {
                if (Hist_Target_ASR_MSCW.at(e).GetNbinsX()<=1200) break;
        ...
        for (int e=0;e<N_energy_bins;e++) {
                double chi2_best = 0.;
                double delta_rms_best = 1000.;
                for (int n_iter = 20;n_iter<=200;n_iter++) {
                        ...
                        chi2 = GetChi2(&Hist_Target_ASR_MSCW.at(e), &Hist_Target_ABkg_MSCW.at(e),-1.,10.); // optimize the modeling in SR
                        if (chi2_best<chi2) {
                            chi2_best = chi2;
                            N_iter.at(e) = n_iter;
                        } 
                }
                std::cout << "Target, e " << e << ", N_iter.at(e) = " << N_iter.at(e) << std::endl;
                ...
                for (int n_iter = 20;n_iter<=200;n_iter++) {
                        ...
                        offset_begin = ShiftAndNormalize(&Hist_Target_ASR_MSCW.at(e),&Hist_Target_ABkgTemp_MSCW.at(e),&Hist_Target_ABkg_MSCW.at(e),offset_begin,-1.,Norm_Upper,true);
                        bool DoShift = false;
                        if (energy_bins[e]>=1000) DoShift = true;
                        offset_begin = ShiftAndNormalize(&Hist_Target_SR_MSCW.at(e),&Hist_Target_BkgTemp_MSCW.at(e),&Hist_Target_Bkg_MSCW.at(e),offset_begin,Norm_Lower,Norm_Upper,DoShift);
                        chi2 = GetChi2(&Hist_Target_SR_MSCW.at(e), &Hist_Target_Bkg_MSCW.at(e),Norm_Lower,Norm_Upper);
                        if (chi2_best<chi2) {
                            chi2_best = chi2;
                            N_iter.at(e) = n_iter;
                        } 
        ...
        for (int e=0;e<N_energy_bins;e++) {
                ...
                for (int n_iter = 20;n_iter<=200;n_iter++) {
                        ...
                        chi2 = GetChi2(&Hist_Dark_ASR_MSCW.at(e), &Hist_Dark_ABkg_MSCW.at(e),-1.,10.); // optimize the modeling in SR
                        if (chi2_best<chi2) {
                            chi2_best = chi2;
                            N_iter.at(e) = n_iter;
                        } 
                }
                std::cout << "Dark, e " << e << ", N_iter.at(e) = " << N_iter.at(e) << std::endl;
                ...
                for (int n_iter = 20;n_iter<=200;n_iter++) {
                        ...
                        bool DoShift = false;
                        if (energy_bins[e]>=1000) DoShift = true;
                        offset_begin = ShiftAndNormalize(&Hist_Dark_SR_MSCW.at(e),&Hist_Dark_BkgTemp_MSCW.at(e),&Hist_Dark_Bkg_MSCW.at(e),offset_begin,Norm_Lower,Norm_Upper,DoShift);
                        chi2 = GetChi2(&Hist_Dark_SR_MSCW.at(e), &Hist_Dark_Bkg_MSCW.at(e),Norm_Lower,Norm_Upper);
                        if (chi2_best<chi2) {
                            chi2_best = chi2;
                            N_iter.at(e) = n_iter;
                        } 
        ...
        for (int e=0;e<N_energy_bins;e++) {
                ...
                for (int i=0;i<Hist_Target_SR_MSCW.at(e).GetNbinsX();i++) {
                        if (Hist_Dark_SR_MSCW.at(e).GetBinCenter(i+1)>1.) 
