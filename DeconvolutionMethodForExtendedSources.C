...
std::pair <double,double> FindConvergeEndpoints(double energy)
{
    //return std::make_pair(1.9,1.0); // 0.28 TeV
    //return std::make_pair(2.0,1.1); // 0.5 TeV
    //if (energy>1100. && energy<1200.) return std::make_pair(2.1,1.2); // 1 TeV
    //return std::make_pair(2.3,1.3); // 2 TeV
    //return std::make_pair(2.5,1.6); // 4 TeV
    //
if (energy>=200&&energy<237) return std::make_pair(1.5,1.4);
if (energy>=237&&energy<282) return std::make_pair(1.4,1.35);
if (energy>=282&&energy<335) return std::make_pair(1.7,1.6);
if (energy>=335&&energy<398) return std::make_pair(1.9,1.3);
if (energy>=398&&energy<473) return std::make_pair(1.9,1.35);
if (energy>=473&&energy<562) return std::make_pair(2.1,1.05);
if (energy>=562&&energy<667) return std::make_pair(1.8,1.55);
if (energy>=667&&energy<794) return std::make_pair(1.9,1.4);
if (energy>=794&&energy<943) return std::make_pair(2.1,1.25);
if (energy>=943&&energy<1122) return std::make_pair(2.1,1.55);
if (energy>=1122&&energy<1332) return std::make_pair(1.9,1.35);
if (energy>=1332&&energy<1585) return std::make_pair(2,1.45);
if (energy>=1585&&energy<1882) return std::make_pair(2.3,1.25);
if (energy>=1882&&energy<2239) return std::make_pair(2.1,1.35);
if (energy>=2239&&energy<3162) return std::make_pair(2.3,1.25);
if (energy>=3162&&energy<4467) return std::make_pair(2.2,1.25);
if (energy>=4467&&energy<6310) return std::make_pair(2.3,1.5);
if (energy>=6310&&energy<8913) return std::make_pair(2.7,1.15);

}
double FindConvergeThreshold(TH1* Hist_SR, TH1* Hist_Bkg, TH1* Hist_Bkg_Temp, double endpoint_up)
{
    double threshold = 0.;
    double chi2_best = 0.;
    double norm_best = 0.;
    ...
    {
        double try_threshold = -2.+(endpoint_up+2)*double(th)/100.;
        double amplitude = endpoint_up-try_threshold;
        double chi2 = 0.;
        double norm = 0.;
        for (int i=0;i<Hist_Bkg->GetNbinsX();i++)
        {
...
double FindConvergeAmplitude(TH1* Hist_SR, TH1* Hist_Bkg, TH1* Hist_Bkg_Temp, double threshold, double init_amplitude,int type)
{
    ...
    {
        double try_amplitude = 0. + (mean-MSCW_cut_lower)*double(amp)/100.;
        double norm = 0.;
        double chi2 = 0.;
        for (int i=0;i<Hist_Bkg->GetNbinsX();i++)
        {
...
void ShiftAndNormalizeAndConverge(TH1* Hist_SR, TH1* Hist_BkgTemp, TH1* Hist_Bkg, double endpoint_0, double endpoint_1, bool doConverge) {
    double shift_fit = 0;
    ...
    for (int fit=0;fit<100;fit++) {
            ...
            if (doConverge) Converge(Hist_Bkg,endpoint_0,endpoint_1,0);
            //double chi2_temp = 1./pow(estimated_rms-Hist_Bkg->GetRMS(),2);
            //double chi2_temp = GetChi2(Hist_SR, Hist_Bkg,false,1)+1./pow(estimated_rms-Hist_Bkg->GetRMS(),2);
            double chi2_temp = GetChi2(Hist_SR, Hist_Bkg,false,1);
...
double PredictNextLayer(TH1* Hist_SR, TH1* Hist_SR_Previous,TH1* Hist_Bkg_Previous, TH1* Hist_Bkg, double energy, double endpoint_0, double endpoint_1, bool useOldSR, bool isUnblinded, bool doConverge)
{
    ...
    SR_Niter = FindNIteration(Hist_SR,&Hist_Bkg_Privous_Adapt,Hist_Bkg,&Hist_Bkg_Temp,&Hist_Kernel,estimated_kernel_rms,0,SR_Niter,false,1);
    std::pair <double,double> kernel_rms = FindRMS(Hist_SR,&Hist_Bkg_Privous_Adapt,Hist_Bkg,&Hist_Bkg_Temp,&Hist_Kernel,estimated_kernel_rms,0,SR_Niter,isUnblinded,1);
    //if (!isEndpointFitting)
    //{
    //    std::cout << "e " << energy << ", SR_Niter = " << SR_Niter << std::endl;
    //    std::cout << "e " << energy << ", SR_Kernel_RMS = " << kernel_rms.first << std::endl;
    //    std::cout << "e " << energy << ", unblinded SR_Kernel_RMS = " << kernel_rms.second << std::endl;
    //}
    ...
    //ShiftAndNormalizeAndConverge(Hist_SR,&Hist_Bkg_Temp,Hist_Bkg,endpoint_0,endpoint_1,doConverge);
    std::pair <bool,std::pair <double,double>> offset = ShiftAndNormalize(Hist_SR,&Hist_Bkg_Temp,Hist_Bkg,true,isUnblinded,2);
    MakeSmoothSplineFunction(Hist_Bkg);
    std::pair <double,double> converge = FindConvergeEndpoints(energy);
    converge.first = endpoint_0;
    converge.second = endpoint_1;
    //if (!isEndpointFitting)
    //{
    //    std::cout << "e " << energy << ", Use endpoint_0 = " << converge.first << std::endl;
    //    std::cout << "e " << energy << ", Use endpoint_1 = " << converge.second << std::endl;
    //}
    if (doConverge) Converge(Hist_Bkg,converge.first,converge.second,energy);
    //if (!isEndpointFitting)
    //{
    //    std::cout << "e " << energy << ", True Mean = " << Hist_SR->GetMean() << std::endl;
    //    std::cout << "e " << energy << ", True RMS = " << Hist_SR->GetRMS() << std::endl;
    //    std::cout << "e " << energy << ", Bkg Mean = " << Hist_Bkg->GetMean() << std::endl;
    //    std::cout << "e " << energy << ", Bkg RMS = " << Hist_Bkg->GetRMS() << std::endl;
    //}
        ...
        for (int e=0;e<N_energy_bins;e++)
        {
		    ...
		    TH1* i_hEffAreaP = ( TH1* )getEffAreaHistogram(input_file,Sublist.at(subrun)[run]);
                ...
                for (int s=1;s<Number_of_CR;s++)
                {
                    ...
                    double this_kernel_rms = PredictNextLayer(&Hist_Target_CR_MSCW.at(e).at(s),&Hist_Target_CR_MSCW.at(e).at(s-1),&Hist_Target_BkgCR_MSCW.at(e).at(s-1),&Hist_Target_BkgCR_MSCW.at(e).at(s),energy_bins[e],-99.,-99.,useOldSR,isUnblinded,doConverge);
                ...
                for (int ep0=0;ep0<50;ep0++) 
                {
                    ...
                    for (int ep1=0;ep1<50;ep1++) 
                    {
                        ...
                        for (int s=1;s<Number_of_CR;s++)
                        {
                            ...
                            Converge(&Hist_Bkg_Tmp,tmp_endpoint_0,tmp_endpoint_1,energy_bins[e]);
                ...
                if (UnblindThisAnalysis) // find end points
                {
                    ...
                    for (int s=0;s<Number_of_SR;s++)
                    {
                        ...
                        if (s==0)
                        {
                            bin0 = Hist_Target_Mean.at(e).FindBin(0.5*(MSCL_signal_cut_lower[0]+MSCL_signal_cut_upper[0]));
                            estimated_amplitude = Hist_Target_Amplitude.at(e).GetBinContent(bin0);
                            estimated_mean = Hist_Target_Mean.at(e).GetBinContent(bin0);
                            estimated_rms = Hist_Target_RMS.at(e).GetBinContent(bin0);
                            estimated_kernel_rms = Hist_Kernel_RMS.at(e).GetBinContent(bin0);
                            estimated_rms_err = Hist_Target_RMS.at(e).GetBinError(bin0);
                            bin0 = Hist_Target_Mean.at(e).FindBin(0.5*(MSCL_control_cut_lower[Number_of_CR-1]+MSCL_control_cut_upper[Number_of_CR-1]));
                            estimated_rms_previous = Hist_Target_RMS.at(e).GetBinContent(bin0);
                            bin0 = Hist_Target_Mean.at(e).FindBin(0.5*(MSCL_signal_cut_lower[0]+MSCL_signal_cut_upper[0]));
                            if (TString(target)=="Proton")
                            {
                            ...
                            useOldSR = true; isUnblinded = true; doConverge = false;
                            double this_kernel_rms = PredictNextLayer(&Hist_Target_SR_MSCW.at(e).at(0),&Hist_Target_CR_MSCW.at(e).at(Number_of_CR-1),&Hist_Target_BkgCR_MSCW.at(e).at(Number_of_CR-1),&Hist_Target_BkgSR_MSCW.at(e).at(0),energy_bins[e],CR_endpoint_0_best,CR_endpoint_1_best,useOldSR,isUnblinded,doConverge);
                        }
                        else
                        {
                            bin0 = Hist_Target_Mean.at(e).FindBin(0.5*(MSCL_control_cut_lower[s]+MSCL_control_cut_upper[s]));
                            ...
                            useOldSR = true; isUnblinded = true; doConverge = false;
                            double this_kernel_rms = PredictNextLayer(&Hist_Target_SR_MSCW.at(e).at(s),&Hist_Target_SR_MSCW.at(e).at(s-1),&Hist_Target_BkgSR_MSCW.at(e).at(s-1),&Hist_Target_BkgSR_MSCW.at(e).at(s),energy_bins[e],CR_endpoint_0_best,CR_endpoint_1_best,useOldSR,isUnblinded,doConverge);
                        }
                        ...
                        for (int ep1=0;ep1<50;ep1++) 
                        {
                            ...
                            for (int s=0;s<Number_of_SR;s++)
                            {
                                ...
                                if (s==0)
                                {
                                    ...
                                    //double this_kernel_rms = PredictNextLayer(&Hist_Target_SR_MSCW.at(e).at(0),&Hist_Target_CR_MSCW.at(e).at(Number_of_CR-1),&Hist_Target_BkgCR_MSCW.at(e).at(Number_of_CR-1),&Hist_Target_BkgSR_MSCW.at(e).at(0),energy_bins[e],tmp_endpoint_0,tmp_endpoint_1,useOldSR,isUnblinded,doConverge);
                                }
                                else
                                {
                                    bin0 = Hist_Target_Mean.at(e).FindBin(0.5*(MSCL_control_cut_lower[s]+MSCL_control_cut_upper[s]));
                                    ...
                                    //double this_kernel_rms = PredictNextLayer(&Hist_Target_SR_MSCW.at(e).at(s),&Hist_Target_SR_MSCW.at(e).at(s-1),&Hist_Target_BkgSR_MSCW.at(e).at(s-1),&Hist_Target_BkgSR_MSCW.at(e).at(s),energy_bins[e],tmp_endpoint_0,tmp_endpoint_1,useOldSR,isUnblinded,doConverge);
                                }

                                TH1D Hist_Bkg_Tmp = TH1D("Hist_Bkg_Tmp","",Hist_Target_BkgCR_MSCW.at(e).at(0).GetNbinsX(),MSCW_plot_lower,MSCW_plot_upper);
                                Hist_Bkg_Tmp.Reset();
                                Hist_Bkg_Tmp.Add(&Hist_Target_BkgSR_MSCW.at(e).at(s));

                                Converge(&Hist_Bkg_Tmp,tmp_endpoint_0,tmp_endpoint_1,energy_bins[e]);

                                Hist_SR_SumSRs.Add(&Hist_Target_SR_MSCW.at(e).at(s));
                                Hist_Bkg_SumSRs.Add(&Hist_Bkg_Tmp);
                            ...
                            //for (int s=0;s<Number_of_SR;s++)
                            //{
                            //    chi2 += GetChi2(&Hist_Target_SR_MSCW.at(e).at(s),&Hist_Target_BkgSR_MSCW.at(e).at(s),true,1);
                            //}
                            if (chi2_best<chi2) {
                                chi2_best = chi2;
                ...
                for (int c=0;c<Number_of_CR-1;c++)
                {
                    ...
                    bool isAdaptive = false;
                    if (c==0) isAdaptive = true;
                    useOldSR = true; isUnblinded = true; doConverge = true;
                    double this_kernel_rms = PredictNextLayer(&Hist_Target_CR_MSCW.at(e).at(c+1),&Hist_Target_CR_MSCW.at(e).at(c),&Hist_Target_BkgCR_MSCW.at(e).at(c),&Hist_Target_BkgCR_MSCW.at(e).at(c+1),energy_bins[e],endpoint_0_best,endpoint_1_best,useOldSR,isUnblinded,doConverge);
                ...
                double this_kernel_rms = PredictNextLayer(&Hist_Target_SR_MSCW.at(e).at(0),&Hist_Target_CR_MSCW.at(e).at(Number_of_CR-1),&Hist_Target_BkgCR_MSCW.at(e).at(Number_of_CR-1),&Hist_Target_BkgSR_MSCW.at(e).at(0),energy_bins[e],endpoint_0_best,endpoint_1_best,useOldSR,isUnblinded,doConverge);
                ...
                for (int s=1;s<Number_of_SR;s++)
                {
                    ...
                    double this_kernel_rms = PredictNextLayer(&Hist_Target_SR_MSCW.at(e).at(s),&Hist_Target_SR_MSCW.at(e).at(s-1),&Hist_Target_BkgSR_MSCW.at(e).at(s-1),&Hist_Target_BkgSR_MSCW.at(e).at(s),energy_bins[e],endpoint_0_best,endpoint_1_best,useOldSR,isUnblinded,doConverge);
