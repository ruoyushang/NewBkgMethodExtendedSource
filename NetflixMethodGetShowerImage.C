...
//double energy_bins[N_energy_bins+1] = {pow(10,2.0),pow(10,2.3),pow(10,3.0),pow(10,4.0)};
...
double roi_1_ra = 0.;
double roi_1_dec = 0.;
double roi_1_radius = 0.;
double roi_2_ra = 0.;
double roi_2_dec = 0.;
double roi_2_radius = 0.;
...
bool FoV() {
    //if (CoincideWithBrightStars(ra_sky,dec_sky) && Theta2_cut_lower>0.) return false;
    //if (R2off<Theta2_cut_lower) return false;
    //if (R2off>Theta2_cut_upper) return false;
...
bool RoIFoV_1() {
    double x = ra_sky-roi_1_ra;
    double y = dec_sky-roi_1_dec;
    double radius = pow(x*x+y*y,0.5);
    if (radius>roi_1_radius) return false;
    return true;
}
bool RoIFoV_2() {
    double x = ra_sky-roi_2_ra;
    double y = dec_sky-roi_2_dec;
    double radius = pow(x*x+y*y,0.5);
    if (radius>roi_2_radius) return false;
    return true;
}
...
void FillAcceptanceMap(TH2* skymap_output, double exposure)
{
    ...
    for (int binx_o=1; binx_o<=skymap_output->GetNbinsX(); binx_o++)
    {
        ...
        {
            double binx_o_ra = skymap_output->GetXaxis()->GetBinCenter(binx_o);
            double biny_o_dec = skymap_output->GetYaxis()->GetBinCenter(biny_o);
            //if (CoincideWithBrightStars(binx_o_ra,biny_o_dec) && Theta2_cut_lower>0.) continue;
            double distance_to_source = pow((binx_o_ra-mean_tele_point_ra)*(binx_o_ra-mean_tele_point_ra)+(biny_o_dec-mean_tele_point_dec)*(biny_o_dec-mean_tele_point_dec),0.5);
            if (distance_to_source<mask_radius) continue;
            ...
            double weight_o = TF1_RadialAcc->Eval(bin_o_r_in_camera);
            skymap_output->Fill(binx_o_ra,biny_o_dec,weight_o*exposure);
        }
    }
}
void FillRingOFFMap(TH2* skymap_output, TH2* skymap_input, TH2* skymap_acceptance, double inner_radius, double outer_radius)
{
    ...
    double mask_radius_i = roi_radius;
    for (int binx_o=1; binx_o<=skymap_output->GetNbinsX(); binx_o++)
    {
        for (int biny_o=1; biny_o<=skymap_output->GetNbinsY(); biny_o++)
        {
            ...
            double biny_o_dec = skymap_output->GetYaxis()->GetBinCenter(biny_o);
            //if (CoincideWithBrightStars(binx_o_ra,biny_o_dec) && Theta2_cut_lower>0.) continue;
            double distance_to_source_o = pow((binx_o_ra-roi_ra)*(binx_o_ra-roi_ra)+(biny_o_dec-roi_dec)*(biny_o_dec-roi_dec),0.5);
            if (distance_to_source_o<mask_radius_o) continue;
            double bin_o_x_in_camera = binx_o_ra-run_tele_point_ra;
            double bin_o_y_in_camera = biny_o_dec-run_tele_point_dec;
            double bin_o_r_in_camera = pow(bin_o_x_in_camera*bin_o_x_in_camera+bin_o_y_in_camera*bin_o_y_in_camera,0.5);
            double weight_o = skymap_acceptance->GetBinContent(binx_o,biny_o);
            double weight_ring = 0.;
            double nbins_included = 0.;
            for (int binx_i=1; binx_i<=skymap_input->GetNbinsX(); binx_i++)
            {
                for (int biny_i=1; biny_i<=skymap_input->GetNbinsY(); biny_i++)
                {
                    double binx_i_ra = skymap_input->GetXaxis()->GetBinCenter(binx_i);
                    double biny_i_dec = skymap_input->GetYaxis()->GetBinCenter(biny_i);
                    //if (CoincideWithBrightStars(binx_i_ra,biny_i_dec) && Theta2_cut_lower>0.) continue;
                    double distance_to_source_i = pow((binx_i_ra-roi_ra)*(binx_i_ra-roi_ra)+(biny_i_dec-roi_dec)*(biny_i_dec-roi_dec),0.5);
                    ...
                    {
                        double bin_i_x_in_camera = binx_i_ra-run_tele_point_ra;
                        double bin_i_y_in_camera = biny_i_dec-run_tele_point_dec;
                        double bin_i_r_in_camera = pow(bin_i_x_in_camera*bin_i_x_in_camera+bin_i_y_in_camera*bin_i_y_in_camera,0.5);
                        double weight_i = skymap_acceptance->GetBinContent(binx_i,biny_i);
                        if (weight_i!=0.)
                        {
                            double weight_add = weight_o/weight_i*skymap_input->GetBinContent(binx_i,biny_i);
                            weight_ring += weight_add;
                            nbins_included += 1.;
                        }
                    }
                }
            }
            if (nbins_included>0.) skymap_output->Fill(binx_o_ra,biny_o_dec,weight_ring/nbins_included);
        }
    }
}
double GetAreaAlphaWeighted(double rov_radius, double ring_radius)
{
    ...
    for (int i=0;i<200;i++)
    {
        ...
        for (int j=0;j<200;j++)
        {
            ...
            //if (CoincideWithBrightStars(bin_ra,bin_dec) && Theta2_cut_lower>0.) continue;
...
void NetflixMethodGetShowerImage(string target_data, double PercentCrab, double tel_elev_lower_input, double tel_elev_upper_input, double Elev_diff, double NSB_diff, bool isON, double MSCW_cut_input, double MSCL_cut_input, double Theta2_cut_input)
{
    ...
    MSCW_cut_upper = MSCW_cut_input;
    MSCL_cut_upper = MSCL_cut_input;
    ...
    roi_radius = 0.15;
    roi_1_ra = mean_tele_point_ra;
    roi_1_dec = mean_tele_point_dec;
    roi_1_radius = 0.15;
    roi_2_ra = mean_tele_point_ra;
    roi_2_dec = mean_tele_point_dec;
    roi_2_radius = 0.15;
    ...
    if (TString(target).Contains("1ES1218")) 
    {
        roi_radius = 0.15;
        roi_1_ra = 185.360;
        roi_1_dec = 30.191;
    ...
    vector<TH1D> Hist_Data_SR_RoIRing_Energy;
    vector<TH1D> Hist_Data_CR_RoI_Energy;
    vector<TH1D> Hist_Data_CR_RoIRing_Energy;
    vector<TH1D> Hist_Data_SR_RoI_1_Energy;
    vector<TH1D> Hist_Data_CR_RoI_1_Energy;
    vector<TH1D> Hist_Data_SR_RoI_2_Energy;
    vector<TH1D> Hist_Data_CR_RoI_2_Energy;
    ...
    vector<TH1D> Hist_Data_CR_RoV0Ring_Energy;
    vector<TH1D> Hist_Data_CR_RoV1Ring_Energy;
    vector<TH1D> Hist_Data_CR_RoV2Ring_Energy;
    vector<TH1D> Hist_Data_CR_RoV3Ring_Energy;
    vector<TH1D> Hist_Data_CR_RoV4Ring_Energy;
    ...
    vector<TH2D> Hist_Data_SR_Skymap2;
    vector<TH2D> Hist_Data_RingOFF_Skymap2;
    vector<TH1D> Hist_Data_SR_Skymap2_Theta2;
    vector<TH1D> Hist_Data_RingOFF_Skymap2_Theta2;
    vector<TH2D> Hist_Acceptance_Skymap2;
    ...
    for (int e=0;e<N_energy_bins;e++) 
    {
        ...
        Hist_Data_SR_RoIRing_Energy.push_back(TH1D("Hist_Data_SR_RoIRing_Energy_ErecS"+TString(e_low)+TString("to")+TString(e_up),"",N_energy_fine_bins,energy_fine_bins));
        ...
        Hist_Data_CR_RoIRing_Energy.push_back(TH1D("Hist_Data_CR_RoIRing_Energy_ErecS"+TString(e_low)+TString("to")+TString(e_up),"",N_energy_fine_bins,energy_fine_bins));
        Hist_Data_SR_RoI_1_Energy.push_back(TH1D("Hist_Data_SR_RoI_1_Energy_ErecS"+TString(e_low)+TString("to")+TString(e_up),"",N_energy_fine_bins,energy_fine_bins));
        Hist_Data_CR_RoI_1_Energy.push_back(TH1D("Hist_Data_CR_RoI_1_Energy_ErecS"+TString(e_low)+TString("to")+TString(e_up),"",N_energy_fine_bins,energy_fine_bins));
        Hist_Data_SR_RoI_2_Energy.push_back(TH1D("Hist_Data_SR_RoI_2_Energy_ErecS"+TString(e_low)+TString("to")+TString(e_up),"",N_energy_fine_bins,energy_fine_bins));
        Hist_Data_CR_RoI_2_Energy.push_back(TH1D("Hist_Data_CR_RoI_2_Energy_ErecS"+TString(e_low)+TString("to")+TString(e_up),"",N_energy_fine_bins,energy_fine_bins));
        ...
        Hist_Data_CR_RoV0Ring_Energy.push_back(TH1D("Hist_Data_CR_RoV0Ring_Energy_ErecS"+TString(e_low)+TString("to")+TString(e_up),"",N_energy_fine_bins,energy_fine_bins));
        Hist_Data_CR_RoV1Ring_Energy.push_back(TH1D("Hist_Data_CR_RoV1Ring_Energy_ErecS"+TString(e_low)+TString("to")+TString(e_up),"",N_energy_fine_bins,energy_fine_bins));
        Hist_Data_CR_RoV2Ring_Energy.push_back(TH1D("Hist_Data_CR_RoV2Ring_Energy_ErecS"+TString(e_low)+TString("to")+TString(e_up),"",N_energy_fine_bins,energy_fine_bins));
        Hist_Data_CR_RoV3Ring_Energy.push_back(TH1D("Hist_Data_CR_RoV3Ring_Energy_ErecS"+TString(e_low)+TString("to")+TString(e_up),"",N_energy_fine_bins,energy_fine_bins));
        Hist_Data_CR_RoV4Ring_Energy.push_back(TH1D("Hist_Data_CR_RoV4Ring_Energy_ErecS"+TString(e_low)+TString("to")+TString(e_up),"",N_energy_fine_bins,energy_fine_bins));
        ...
        Hist_Data_SR_Skymap2.push_back(TH2D("Hist_Data_SR_Skymap2_ErecS"+TString(e_low)+TString("to")+TString(e_up),"",150,mean_tele_point_ra-3,mean_tele_point_ra+3,150,mean_tele_point_dec-3,mean_tele_point_dec+3));
        Hist_Data_RingOFF_Skymap2.push_back(TH2D("Hist_Data_RingOFF_Skymap2_ErecS"+TString(e_low)+TString("to")+TString(e_up),"",150,mean_tele_point_ra-3,mean_tele_point_ra+3,150,mean_tele_point_dec-3,mean_tele_point_dec+3));
        Hist_Acceptance_Skymap2.push_back(TH2D("Hist_Acceptance_Skymap2_ErecS"+TString(e_low)+TString("to")+TString(e_up),"",150,mean_tele_point_ra-3,mean_tele_point_ra+3,150,mean_tele_point_dec-3,mean_tele_point_dec+3));
        Hist_Data_SR_Skymap2_Theta2.push_back(TH1D("Hist_Data_SR_Skymap2_Theta2_ErecS"+TString(e_low)+TString("to")+TString(e_up),"",50,0,10));
        Hist_Data_RingOFF_Skymap2_Theta2.push_back(TH1D("Hist_Data_RingOFF_Skymap2_Theta2_ErecS"+TString(e_low)+TString("to")+TString(e_up),"",50,0,10));
    }
	...
	TH1* i_hEffAreaP = ( TH1* )getEffAreaHistogram(input_file,Data_runlist[run].second);
        ...
        //double area_alpha_RoI = 1.;
        //double area_alpha_RoV0 = 1.;
        //double area_alpha_RoV1 = 1.;
        //double area_alpha_RoV2 = 1.;
        //double area_alpha_RoV3 = 1.;
        //double area_alpha_RoV4 = 1.;
        for (int entry=0;entry<Data_tree->GetEntries();entry++) 
        {
            ...
            if (FoV() || Data_runlist[run].first.find("Proton")!=std::string::npos)
            {
                ...
                Hist_Data_MSCLW.at(energy).Fill(MSCL,MSCW);
                Hist_TrueBkgd_MSCLW.at(energy).Fill(MSCL,MSCW);
            }
            if (SignalSelectionTheta2())
            {
                if (FoV() || Data_runlist[run].first.find("Proton")!=std::string::npos)
                {
                    ...
                    if (RoIRingFoV()) Hist_Data_SR_RoIRing_Energy.at(energy).Fill(ErecS*1000.,area_alpha_RoI);
                    if (RoIFoV_1()) Hist_Data_SR_RoI_1_Energy.at(energy).Fill(ErecS*1000.);
                    if (RoIFoV_2()) Hist_Data_SR_RoI_2_Energy.at(energy).Fill(ErecS*1000.);
                    ...
                    Hist_Data_SR_Skymap2.at(energy).Fill(ra_sky,dec_sky);
                    Hist_Data_SR_CameraFoV.at(energy_fine).Fill(Xoff,Yoff);
                    Hist_Data_SR_Skymap_Galactic.at(energy_fine).Fill(evt_l_b.first,evt_l_b.second);
                }
            }
            if (ControlSelectionTheta2())
            {
                if (FoV() || Data_runlist[run].first.find("Proton")!=std::string::npos)
                {
                    int binx = Hist_Dark_SR_CameraFoV.at(energy_fine).GetXaxis()->FindBin(R2off);
                    int biny = Hist_Dark_SR_CameraFoV.at(energy_fine).GetYaxis()->FindBin(Phioff);
                    double dark_cr_content = Hist_Dark_CR_CameraFoV.at(energy_fine).GetBinContent(binx,biny);
                    double dark_sr_content = Hist_Dark_SR_CameraFoV.at(energy_fine).GetBinContent(binx,biny);
                    double weight = 0.;
                    if (dark_cr_content>0.) weight = dark_sr_content/dark_cr_content;
                    int bin_e = Hist_Dark_SR_Energy.at(energy).FindBin(ErecS*1000.);
                    double dark_cr_content_e = Hist_Dark_CR_Energy.at(energy).GetBinContent(bin_e);
                    double dark_sr_content_e = Hist_Dark_SR_Energy.at(energy).GetBinContent(bin_e);
                    double weight_e = 0.;
                    ...
                    if (RoIRingFoV()) Hist_Data_CR_RoIRing_Energy.at(energy).Fill(ErecS*1000.,weight_e);
                    if (RoIFoV_1()) Hist_Data_CR_RoI_1_Energy.at(energy).Fill(ErecS*1000.,weight_e);
                    if (RoIFoV_2()) Hist_Data_CR_RoI_2_Energy.at(energy).Fill(ErecS*1000.,weight_e);
                    ...
                    if (RoVRingFoV(0.1+0.0)) Hist_Data_CR_RoV0Ring_Energy.at(energy).Fill(ErecS*1000.,weight_e);
                    if (RoVRingFoV(0.5+0.0)) Hist_Data_CR_RoV1Ring_Energy.at(energy).Fill(ErecS*1000.,weight_e);
                    if (RoVRingFoV(1.0+0.0)) Hist_Data_CR_RoV2Ring_Energy.at(energy).Fill(ErecS*1000.,weight_e);
                    if (RoVRingFoV(1.5+0.0)) Hist_Data_CR_RoV3Ring_Energy.at(energy).Fill(ErecS*1000.,weight_e);
                    if (RoVRingFoV(2.0+0.0)) Hist_Data_CR_RoV4Ring_Energy.at(energy).Fill(ErecS*1000.,weight_e);
                    ...
                    Hist_Data_CR_CameraFoV_Theta2_Raw.at(energy_fine).Fill(R2off,1.);
                    Hist_Data_CR_Skymap.at(energy_fine).Fill(ra_sky,dec_sky,weight);
                    Hist_Data_CR_Skymap_Raw.at(energy_fine).Fill(ra_sky,dec_sky,1.);
                    Hist_Data_CR_Skymap_Galactic.at(energy_fine).Fill(evt_l_b.first,evt_l_b.second,weight);
                    Hist_Data_CR_CameraFoV.at(energy_fine).Fill(Xoff,Yoff,weight);
                    Hist_Data_CR_CameraFoV_Raw.at(energy_fine).Fill(Xoff,Yoff,1.);
                }
            }
        }
        input_file->Close();
        ...
        {
            FillAcceptanceMap(&Hist_Acceptance_Skymap2.at(e_bin),(time_1-time_0));
        }
    }
	...
	TH1* i_hEffAreaP = ( TH1* )getEffAreaHistogram(input_file,PhotonMC_runlist[run].second);
        ...
        for (int entry=0;entry<Data_tree->GetEntries();entry++) 
        {
            ...
            if (FoV() && GammaFoV())
            {
                ...
                Hist_Data_MSCLW.at(energy).Fill(MSCL,MSCW,photon_weight);
            }
            if (SignalSelectionTheta2())
            {
                if (FoV() && GammaFoV())
                {
                    ...
                    if (RoIRingFoV()) Hist_Data_SR_RoIRing_Energy.at(energy).Fill(ErecS*1000.,photon_weight);
                    if (RoIFoV_1()) Hist_Data_SR_RoI_1_Energy.at(energy).Fill(ErecS*1000.,photon_weight);
                    if (RoIFoV_2()) Hist_Data_SR_RoI_2_Energy.at(energy).Fill(ErecS*1000.,photon_weight);
                    ...
                    Hist_Data_SR_Skymap2.at(energy).Fill(ra_sky,dec_sky,photon_weight);
            ...
            if (ControlSelectionTheta2())
            {
                if (FoV() && GammaFoV())
                {
                    ...
                    if (RoIRingFoV()) Hist_Data_CR_RoIRing_Energy.at(energy).Fill(ErecS*1000.,weight_e*photon_weight);
                    if (RoIFoV_1()) Hist_Data_CR_RoI_1_Energy.at(energy).Fill(ErecS*1000.,weight_e*photon_weight);
                    if (RoIFoV_2()) Hist_Data_CR_RoI_2_Energy.at(energy).Fill(ErecS*1000.,weight_e*photon_weight);
                    ...
                    if (RoVRingFoV(0.1+0.0)) Hist_Data_CR_RoV0Ring_Energy.at(energy).Fill(ErecS*1000.,weight_e*photon_weight);
                    if (RoVRingFoV(0.5+0.0)) Hist_Data_CR_RoV1Ring_Energy.at(energy).Fill(ErecS*1000.,weight_e*photon_weight);
                    if (RoVRingFoV(1.0+0.0)) Hist_Data_CR_RoV2Ring_Energy.at(energy).Fill(ErecS*1000.,weight_e*photon_weight);
                    if (RoVRingFoV(1.5+0.0)) Hist_Data_CR_RoV3Ring_Energy.at(energy).Fill(ErecS*1000.,weight_e*photon_weight);
                    if (RoVRingFoV(2.0+0.0)) Hist_Data_CR_RoV4Ring_Energy.at(energy).Fill(ErecS*1000.,weight_e*photon_weight);
                    ...
                    Hist_Data_CR_CameraFoV_Theta2_Raw.at(energy_fine).Fill(R2off,photon_weight);
                    Hist_Data_CR_Skymap.at(energy_fine).Fill(ra_sky,dec_sky,weight*photon_weight);
                    Hist_Data_CR_Skymap_Raw.at(energy_fine).Fill(ra_sky,dec_sky,photon_weight);
                    Hist_Data_CR_Skymap_Galactic.at(energy_fine).Fill(evt_l_b.first,evt_l_b.second,weight*photon_weight);
                }
            }
        }
        input_file->Close();
    ...
    //for (int e=0;e<N_energy_bins;e++) 
    //{
    //    double scale_roi_ring = Hist_Data_CR_RoI_Energy.at(e).Integral()/Hist_Data_CR_RoIRing_Energy.at(e).Integral();
    //    Hist_Data_SR_RoIRing_Energy.at(e).Scale(scale_roi_ring);
    //    double scale_rov0_ring = Hist_Data_CR_RoV0_Energy.at(e).Integral()/Hist_Data_CR_RoV0Ring_Energy.at(e).Integral();
    //    Hist_Data_SR_RoV0Ring_Energy.at(e).Scale(scale_rov0_ring);
    //    double scale_rov1_ring = Hist_Data_CR_RoV1_Energy.at(e).Integral()/Hist_Data_CR_RoV1Ring_Energy.at(e).Integral();
    //    Hist_Data_SR_RoV1Ring_Energy.at(e).Scale(scale_rov1_ring);
    //    double scale_rov2_ring = Hist_Data_CR_RoV2_Energy.at(e).Integral()/Hist_Data_CR_RoV2Ring_Energy.at(e).Integral();
    //    Hist_Data_SR_RoV2Ring_Energy.at(e).Scale(scale_rov2_ring);
    //    double scale_rov3_ring = Hist_Data_CR_RoV3_Energy.at(e).Integral()/Hist_Data_CR_RoV3Ring_Energy.at(e).Integral();
    //    Hist_Data_SR_RoV3Ring_Energy.at(e).Scale(scale_rov3_ring);
    //    double scale_rov4_ring = Hist_Data_CR_RoV4_Energy.at(e).Integral()/Hist_Data_CR_RoV4Ring_Energy.at(e).Integral();
    ...
    for (int e_bin=0;e_bin<N_energy_bins;e_bin++)
    {
        ...
        FillRingOFFMap(&Hist_Data_RingOFF_Skymap2.at(e_bin),&Hist_Data_SR_Skymap2.at(e_bin),&Hist_Acceptance_Skymap2.at(e_bin),roi_radius,roi_radius+0.1);
    ...
    InfoTree.Branch("MSCW_cut_blind",&MSCW_cut_upper,"MSCW_cut_blind/D");
    InfoTree.Branch("MSCL_cut_blind",&MSCL_cut_upper,"MSCL_cut_blind/D");
    ...
    for (int e=0;e<N_energy_bins;e++)
    {
        ...
        Hist_Data_SR_RoIRing_Energy.at(e).Write();
        Hist_Data_CR_RoI_Energy.at(e).Write();
        Hist_Data_SR_RoI_1_Energy.at(e).Write();
        Hist_Data_CR_RoI_1_Energy.at(e).Write();
        Hist_Data_SR_RoI_2_Energy.at(e).Write();
        Hist_Data_CR_RoI_2_Energy.at(e).Write();
        ...
        Hist_Data_SR_Skymap2.at(e).Write();
        Hist_Data_RingOFF_Skymap2.at(e).Write();
        Hist_Data_SR_Skymap2_Theta2.at(e).Write();
        Hist_Data_RingOFF_Skymap2_Theta2.at(e).Write();
    }
    for (int e=0;e<N_energy_fine_bins;e++)
    {
