...
energy_bin = []
energy_bin += [pow(10,2.3)]
energy_bin += [pow(10,2.4)]
energy_bin += [pow(10,2.5)]
energy_bin += [pow(10,2.6)]
energy_bin += [pow(10,2.7)]
energy_bin += [pow(10,2.8)]
energy_bin += [pow(10,2.9)]
energy_bin += [pow(10,3.0)]
energy_bin += [pow(10,3.1)]
energy_bin += [pow(10,3.2)]
energy_bin += [pow(10,3.3)]
energy_bin += [pow(10,3.4)]
energy_bin += [pow(10,3.5)]
energy_bin += [pow(10,3.6)]
energy_bin += [pow(10,3.7)]
energy_bin += [pow(10,3.8)]
...
Hist_Bkgd_Energy = ROOT.TH1D("Hist_Bkgd_Energy","",len(energy_bin)-1,array('d',energy_bin))
Hist_Data_Energy_SumE = ROOT.TH1D("Hist_Data_Energy_SumE","",len(energy_bin)-1,array('d',energy_bin))
Hist_TrueBkgd_Energy_SumE = ROOT.TH1D("Hist_TrueBkgd_Energy_SumE","",len(energy_bin)-1,array('d',energy_bin))
Hist_Bkgd_Energy_SumE = ROOT.TH1D("Hist_Bkgd_Energy_SumE","",len(energy_bin)-1,array('d',energy_bin))
...
def MakeChi2Plot(Hists,legends,colors,title,name,doLogx,doLogy,range_lower,range_upper,syst):
...
def CompareWithOfficalSpectrum(Hist,legends,title_x,title_y,name,y_min,y_max,logx,logy):
...
def PlotsStackedHistograms(tag):
    ...
    title = 'MSCW'
    MakeChi2Plot(Hists,legends,colors,title,plotname,False,False,MSCW_lower_cut,MSCW_blind_cut,-1)
    ...
    title = 'MSCL'
    MakeChi2Plot(Hists,legends,colors,title,plotname,False,False,MSCL_lower_cut,MSCL_blind_cut,-1)
    ...
    MakeChi2Plot(Hists,legends,colors,title,plotname,False,False,0.,1.,-1)
    ...
    title = 'energy [GeV]'
    MakeChi2Plot(Hists,legends,colors,title,plotname,True,True,0.,pow(10,4.0),-1)
...
def SystAsFunctionOfRank():
    ...
    MakeComparisonPlot(Hist_Ratio_Rank,legend_S2B,color_S2B,'n ranks','1-ratio in gamma region','Ratio_Rank%s_%s'%(PercentCrab,folder_tag),0.,0.,False,True)
...
def SystDarkVsMDM():
    ...
    for source in range(0,len(FilePath_Folder0)):
        ...
        legend_S2B += ['%s (%0.1f%%)'%(source_name,100.*(1.-s2b))]
        s2b = 0.
        s2b_err = 0.
        if not "_Crab0" in PercentCrab:
            s2b, s2b_err = Variation_ratio(Hist_TrueBkgd_Theta2_SumE,Hist_Dark_Theta2_SumE,0.2,10.)
...
def SingleSourceSpectrum(source_name_input):
    ...
    CompareWithOfficalSpectrum(Hist_MDM_NormSig_Rate[len(Hist_MDM_Sig_Rate)-1],legend_S2B[len(legend_S2B)-1],'E [GeV]','events/sec/cm^{2}/TeV','SignalFluxOffical_%s_MDM%s_%s'%(source_name_input,PercentCrab,folder_tag),0.,0.,True,True)
...
def SingleSourceSkyMap(source_name_input):
    ...
    for source in range(0,len(FilePath_Folder0)):
        ...
        for hist in range(0,len(Hist_Eigenvalue_real_Rank)):
            Hist_Eigenvalue_real_Rank[hist].GetXaxis().SetRangeUser(0,16)
        MakeComparisonPlot(Hist_Eigenvalue_real_Rank,legend_S2B,color_S2B,'k','Re(#lambda_{k})/Re(#lambda_{0})','ONOFF_Eigenvalue_real_Rank_%s%s_%s'%(source_name,PercentCrab,folder_tag),0.,0.,False,True)
        for hist in range(0,len(Hist_Eigenvalue_imag_Rank)):
            Hist_Eigenvalue_imag_Rank[hist].GetXaxis().SetRangeUser(0,16)
        MakeComparisonPlot(Hist_Eigenvalue_imag_Rank,legend_S2B,color_S2B,'k','Im(#lambda_{k})/Re(#lambda_{0})','ONOFF_Eigenvalue_imag_Rank_%s%s_%s'%(source_name,PercentCrab,folder_tag),0.,0.,False,False)
